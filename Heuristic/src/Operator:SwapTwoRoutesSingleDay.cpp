                            ...
                            if (Swap_length1 != 0 || Swap_length2 != 0)
                            {
                                ...
                                if (pick_position_in_vehicle2 + Swap_length2 <= IRPSolution.Route[pick_day][pick_vehicle2].size() && pick_position_in_vehicle1 + Swap_length1 <= IRPSolution.Route[pick_day][pick_vehicle1].size()) // Make sure the operator does not go outside the range
                                {
                                    ...
                                    if (Swap_length1 <= 3 && Swap_length2 <= 3)
                                    {
                                        ...
                                        vector<vector<double>> NewInventoryLevelRoute1AllowingCapacityViolation;
                                        for (int route1_index = pick_position_in_vehicle1; route1_index < pick_position_in_vehicle1 + Swap_length1; route1_index++)
                                        {
                                            NewDeliveryQuantityRoute1AllowingCapacityViolation.push_back(IRPSolution.DeliveryQuantity[IRPSolution.Route[pick_day][pick_vehicle1][route1_index]]);
                                            NewInventoryLevelRoute1AllowingCapacityViolation.push_back(IRPSolution.InventoryLevel[IRPSolution.Route[pick_day][pick_vehicle1][route1_index]]);
                                        }
                                        assert(NewDeliveryQuantityRoute1AllowingCapacityViolation.size() == Swap_length1);
                                        assert(NewInventoryLevelRoute1AllowingCapacityViolation.size() == Swap_length1);
                                        vector<vector<double>> NewDeliveryQuantityRoute2AllowingCapacityViolation;
                                        vector<vector<double>> NewInventoryLevelRoute2AllowingCapacityViolation;
                                        for (int route2_index = pick_position_in_vehicle2; route2_index < pick_position_in_vehicle2 + Swap_length2; route2_index++)
                                        {
                                            NewDeliveryQuantityRoute2AllowingCapacityViolation.push_back(IRPSolution.DeliveryQuantity[IRPSolution.Route[pick_day][pick_vehicle2][route2_index]]);
                                            NewInventoryLevelRoute2AllowingCapacityViolation.push_back(IRPSolution.InventoryLevel[IRPSolution.Route[pick_day][pick_vehicle2][route2_index]]);
                                        }
                                        assert(NewDeliveryQuantityRoute2AllowingCapacityViolation.size() == Swap_length2);
                                        assert(NewInventoryLevelRoute2AllowingCapacityViolation.size() == Swap_length2);
                                        ...
                                        cout << "intialize all data needed to capture the improving solution" << endl;
                                        cout << "SelectedVehicleLoad1AllowingCapacityViolation: " << CopyVehicleLoadAllowingCapacityViolation[pick_day][pick_vehicle1] << endl;
                                        cout << "SelectedVehicleLoad2AllowingCapacityViolation: " << CopyVehicleLoadAllowingCapacityViolation[pick_day][pick_vehicle2] << endl;
                                        ...
                                        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                        // Get change in delivery quantity and inventory level
                                        int Whether_insert_fail_allowing_capacity_violation = 0; // if insert does not lead to an increase on delivery quantity, this move fails, therefore skip the evalution
                                        // Remove customers from route 1 and route 2
                                        // Make delivery quantity 0 for removed customers
                                        // Record the change in total delivery quantity
                                        for (int route1_index = 0; route1_index < Swap_length1; route1_index++)
                                        ...
                                        for (int route1_index = 0; route1_index < Swap_length1; route1_index++)
                                        {
                                            ...
                                            {
                                                Whether_insert_fail_allowing_capacity_violation = 1;
                                            }
                                            else
                                            {
                                        ...
                                        if (Whether_insert_fail_allowing_capacity_violation == 0)
                                        {
                                            working_solutionCounter++;
                                            double NewRoute1Cost = memory.ConcatenateSwapTwoRoutesSingleDay(
                                                pick_day,
                                            ...
                                            double NewTotalDelivery = IRPSolution.TotalDelivery + ChangeInTotalQuantityAllowingCapacityViolation;
                                            double NewLogisticRatio = NewTotalTransportationCost / NewTotalDelivery;
                                            double temp_LR_objv = Calculate_la_relax_objv(NewLogisticRatio, PenaltyForStockOut, NewStockOutAllowingCapacityViolation, PenaltyMoreThanCapacity, NewVehicleOverloadAllowingCapacityViolation);
                                            //  cout<<"objv_begin:"<<objv_begin<<" temp_LR_objv:" << temp_LR_objv << endl;
                                            if (objv_begin - temp_LR_objv > 0.00001)
                                            {
                                                ...
                                                if (LR_objv - temp_LR_objv > 0.00001)
                                                {
                                                    ...
                                                    for (int route1_index = 0; route1_index < Swap_length1; route1_index++)
                                                    {
                                                        ImpDeliveryQuantityRoute1.push_back(NewDeliveryQuantityRoute1AllowingCapacityViolation[route1_index]);
                                                        ImpInventoryLevelRoute1.push_back(NewInventoryLevelRoute1AllowingCapacityViolation[route1_index]);
                                                    }

                                                    for (int route2_index = 0; route2_index < Swap_length2; route2_index++)
                                                    {
                                                        ImpDeliveryQuantityRoute2.push_back(NewDeliveryQuantityRoute2AllowingCapacityViolation[route2_index]);
                                                        ImpInventoryLevelRoute2.push_back(NewInventoryLevelRoute2AllowingCapacityViolation[route2_index]);
                                                    }
                                                    ImpVehicleLoad = CopyVehicleLoadAllowingCapacityViolation;
                                                    ImpStockOut = NewStockOutAllowingCapacityViolation;
                                                    ImpVehicleOverload = NewVehicleOverloadAllowingCapacityViolation;
                                                    ImpLogisticRatio = NewLogisticRatio;
                                                    ImpTotalTransportationCost = NewTotalTransportationCost;
                                                    ImpTotalDelivery = NewTotalDelivery;
