...
void preprocessing::PopulatePrefixAndSuffix(input &IRPLR, solution &IRPSolution)
{
    ...
    Prefix.clear();
    Suffix.clear();
    TrackSolutionStatus.clear();
    TrackSingleRouteOptimisation.clear();

    for (int i = 0; i < IRPSolution.Route.size(); i++)//day
    {   vector<int> TrackSolutionStatusEachPeriod;
        vector<int> TrackSingleRouteOptimisationEachPeriod;
        vector<vector<double>> PrefixEachPeriod;
        vector<vector<double>> SuffixEachPeriod;
        vector<double> TempTransportationCostPerRoute;
        for (int j = 0; j < IRPSolution.Route[i].size(); j++)//vehicle
        {
            ...
            TrackSolutionStatusEachPeriod.push_back(0); // Initially, all routes need to be processed
            TrackSingleRouteOptimisationEachPeriod.push_back(0); // Initially, no route has been optimised
        }

        Prefix.push_back(PrefixEachPeriod);
        Suffix.push_back(SuffixEachPeriod);
        TrackSolutionStatus.push_back(TrackSolutionStatusEachPeriod);
        TrackSingleRouteOptimisation.push_back(TrackSingleRouteOptimisationEachPeriod);
    }
...
double preprocessing::PopulateSingleRoutePrefixAndSuffix(input &IRPLR, vector<int> & route)
{
    ...
    assert(abs(TransportationCost - TransportationCostForTheDay)<=0.00001);*/
