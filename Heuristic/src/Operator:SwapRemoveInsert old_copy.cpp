#include "lib.h"
int solution_improvement::OperatorSwapRemoveInsert(input &IRPLR, solution &IRPSolution, HGS &Routing, double &PenaltyForStockOut, double &PenaltyMoreThanCapacity, preprocessing &memory,
                                ...
                                if (remove_length != 0 || insert_length != 0)
                                {
                                    ...
                                    else
                                    {
                                        if (pick_unallocated_customer + insert_length <= IRPSolution.UnallocatedCustomers[pick_day].size() && pick_allocated_customer + remove_length <= IRPSolution.Route[pick_day][pick_vehicle].size()) // Make sure the operator does not go outside the range
                                        {
                                            ...
                                            double OldRouteCost = 0;
                                            if (TempRoute[pick_day][pick_vehicle].size() != 0)
                                            {
                                                OldRouteCost = IRPLR.Distance[0][TempRoute[pick_day][pick_vehicle][0] + 1];
                                                OldRouteCost += IRPLR.Distance[TempRoute[pick_day][pick_vehicle][TempRoute[pick_day][pick_vehicle].size() - 1] + 1][0];
                                                for (int test = 0; test < TempRoute[pick_day][pick_vehicle].size() - 1; test++)
                                                {
                                                    OldRouteCost += IRPLR.Distance[TempRoute[pick_day][pick_vehicle][test] + 1][TempRoute[pick_day][pick_vehicle][test + 1] + 1];
                                                }
                                            }
                                            ...
                                            if (remove_length != 0)
                                            {
                                                ...
                                                if (pick_day == 0)
                                                {
                                                    ...
                                                    for (int y = 0; y < TempInventoryLevel[TempUnallocatedCustomers[pick_day][pick_unallocated_customer]].size(); y++)
                                                    {
                                                        ...
                                                        if (fabs(TempInventoryLevel[TempUnallocatedCustomers[pick_day][pick_unallocated_customer]][y] - tempInventory) > 0.00001)
                                                        {
                                                            if (TempInventoryLevel[TempUnallocatedCustomers[pick_day][pick_unallocated_customer]][y] >= 0.0 && tempInventory < -0.00001)
                                                            {
                                                                NewStockOut += -tempInventory;
                                                            }
                                                            else if (TempInventoryLevel[TempUnallocatedCustomers[pick_day][pick_unallocated_customer]][y] < -0.00001 && tempInventory >= 0.0)
                                                            {
                                                                NewStockOut -= -TempInventoryLevel[TempUnallocatedCustomers[pick_day][pick_unallocated_customer]][y];
                                                            }
                                                            else if (TempInventoryLevel[TempUnallocatedCustomers[pick_day][pick_unallocated_customer]][y] < -0.00001 && tempInventory < -0.00001)
                                                            {
                                                ...
                                                if (Whether_insert_fail == 0)
                                                {
                                                    ...
                                                    if (pick_day == 0)
                                                    {
                                                        ...
                                                        for (int y = 0; y < TempInventoryLevel[TempRoute[pick_day][pick_vehicle][pick_allocated_customer]].size(); y++)
                                                        {
                                                            ...
                                                            if (fabs(TempInventoryLevel[TempRoute[pick_day][pick_vehicle][pick_allocated_customer]][y] - AdjustInventoryInsert) > 0.00001)
                                                            {
                                                                if (TempInventoryLevel[TempRoute[pick_day][pick_vehicle][pick_allocated_customer]][y] >= 0.0 && AdjustInventoryInsert < -0.00001)
                                                                {
                                                                    NewStockOut += -AdjustInventoryInsert;
                                                                }
                                                                else if (TempInventoryLevel[TempRoute[pick_day][pick_vehicle][pick_allocated_customer]][y] < -0.00001 && AdjustInventoryInsert >= 0.0)
                                                                {
                                                                    NewStockOut -= -TempInventoryLevel[TempRoute[pick_day][pick_vehicle][pick_allocated_customer]][y];
                                                                }
                                                                else if (TempInventoryLevel[TempRoute[pick_day][pick_vehicle][pick_allocated_customer]][y] < -0.00001 && AdjustInventoryInsert < -0.00001)
                                                                {
                                            ...
                                            if (Whether_insert_fail == 0)
                                            {
                                                if (insert_length != 0)
                                                {
                                                    ...
                                                    int FindingCheapestInsertion = OperatorCheapestInsertion(IRPLR, TempRoute[pick_day][pick_vehicle], pick_day, pick_vehicle, pick_allocated_customer, PenaltyForStockOut, ImprovedTransportationCost, memory); // Finding cheapest insertion
                                                }
                                                ...
                                                // assert(fabs(TempViolationStockOut - NewStockOut) < 0.00001);
                                                //  cout << temp_LR_objv << "," << LR_objv << endl;
