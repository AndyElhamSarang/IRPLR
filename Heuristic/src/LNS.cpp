        ...
        while (OperatorSwapCounter < 50)
        {
            ...
            else
            {
                ...
                if (BestIRP_Solution.LogisticRatio - IRPSolution.LogisticRatio > 0.00001)
                {
                    ...
                    cout << "@Best solution is updated at time:" << update_time << " s,\t with " << "TotalTransportationCost:" << IRPSolution.TotalTransportationCost << ",\t TotalDelivery:" << IRPSolution.TotalDelivery << ",\t LogisticRatio:" << IRPSolution.LogisticRatio << endl;
